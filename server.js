...
const { db, addMeeting, getMeeting, updateMeetingId, getMeetingKey, deleteMeeting, addSearchQuery, getUserSearchHistory, deleteSearchHistoryItem, clearUserSearchHistory } = require('./database');
...
app.post('/api/join', async (req, res) => {
    ...
    const { meeting_url, bot_name, language } = req.body;
    ...
    try {
        ...
        if (recallResponse && recallResponse.success) {
            const botId = recallResponse.id;
            console.log(`[Recall] Bot joined with ID: ${botId}`);
            try {
                await addMeeting(user_id, botId, language || 'auto');
            } catch (dbErr) {
                console.warn(`[Database Warning] Meeting save warning: ${dbErr.message}`);
            }
...
app.post('/api/upload-direct-audio', express.raw({ type: '*/*', limit: '100mb' }), async (req, res) => {
    const user_id = req.user.uid;
    ...
    const meetingId = `direct-rec-${Date.now()}`;
    console.log(`[Direct Record] Audio received (${(req.body.length / 1024 / 1024).toFixed(2)} MB) for user ${user_id}. Meeting ID: ${meetingId}`);

    try {
        await addMeeting(user_id, meetingId, language);

        const { Readable } = require('stream');
        const audioStream = Readable.from(req.body);

        ingestRecording(meetingId, audioStream).catch(err => {
            console.error(`[Direct Record Ingest Error] ${meetingId}:`, err);
        });

        res.json({ success: true, meeting_id: meetingId, message: "Audio uploaded and processing started." });
    } catch (error) {
        console.error(`[Direct Record Error]`, error);
...
// --- SEARCH HISTORY ENDPOINTS (USER-ISOLATED) ---

app.get('/api/search-history', async (req, res) => {
    const user_id = req.user.uid;
    try {
        const history = await getUserSearchHistory(user_id, 10);
...
app.post('/api/search-history', async (req, res) => {
    ...
    try {
        const result = await addSearchQuery(user_id, query);
...
app.delete('/api/search-history/:id', async (req, res) => {
    ...
    const { id } = req.params;
    try {
        const result = await deleteSearchHistoryItem(user_id, id);
        res.json({ success: true, ...result });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.delete('/api/search-history', async (req, res) => {
    const user_id = req.user.uid;
    try {
        const result = await clearUserSearchHistory(user_id);
        res.json({ success: true, ...result });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
function ensureOllamaRunning() {
    exec('tasklist /FI "IMAGENAME eq ollama.exe"', (err, stdout) => {
        if (err || !stdout.includes('ollama.exe')) {
            ...
            ollama.on('error', (err) => {
                console.warn("[Server] Failed to auto-start Ollama (possibly not installed):", err.message);
            });
            ollama.unref();
        } else {
