...
const { storeMeetingKey, getMeetingKey, updateProcessState, getMeeting, db } = require('./database');
...
async function ingestRecording(meetingId, audioStream) {
    ...
    // Create a PassThrough stream for conversion if FFmpeg exists
    const mp3Stream = new PassThrough();
    let streamToEncrypt = audioStream;

    try {
        const command = ffmpeg(audioStream)
            .format('mp3')
            .on('error', (err) => {
                console.warn(`[FFmpeg Warning] ${meetingId}: ${err.message}. Using raw audio stream.`);
            });
        command.pipe(mp3Stream);
        streamToEncrypt = mp3Stream;
    } catch (err) {
        console.warn(`[FFmpeg Warning] FFmpeg unavailable. Encrypting raw audio stream directly.`);
        streamToEncrypt = audioStream;
    }

    // Store credentials securely FIRST so key is in DB immediately
    await storeMeetingKey(meetingId, key, iv);

    // Encrypt the audio stream to disk
    await encryptStreamToFile(streamToEncrypt, filePath, key, iv);
...
async function processMeeting(meetingId) {
    try {
        ...
        const language = meeting ? (meeting.language || 'auto') : 'auto';
