...
function initializeDatabase() {
    db.serialize(() => {
        ...
        // Migration: Add 'language' column to meetings table if it doesn't exist
        db.run("ALTER TABLE meetings ADD COLUMN language TEXT DEFAULT 'auto'", (err) => {
            if (err) {
                if (!err.message.includes("duplicate column name")) {
                    console.error("[Database] Migration Error:", err.message);
                }
            } else {
                console.log("[Database] Migration: Added 'language' column to meetings table.");
            }
        });
        ...
        // 4. Search History Table (User-Isolated)
...
function addMeeting(user_id, bot_id, language = 'auto') {
    return new Promise((resolve, reject) => {
        const stmt = db.prepare("INSERT OR REPLACE INTO meetings (id, user_id, bot_id, created_at, process_state, language) VALUES (?, ?, ?, ?, ?, ?)");
        const now = Date.now();
        stmt.run(bot_id, user_id, bot_id, now, 'initializing', language || 'auto', function (err) {
            if (err) reject(err);
...
function addSearchQuery(user_id, query) {
    return new Promise((resolve, reject) => {
        const cleanQuery = (query || '').trim();
        if (!cleanQuery) return resolve({ success: false, message: 'Query empty' });

        db.run("DELETE FROM search_history WHERE user_id = ? AND LOWER(query) = LOWER(?)", [user_id, cleanQuery], (err) => {
            if (err) return reject(err);

            const stmt = db.prepare("INSERT INTO search_history (user_id, query, created_at) VALUES (?, ?, ?)");
            stmt.run(user_id, cleanQuery, Date.now(), function (err2) {
                if (err2) reject(err2);
                else resolve({ id: this.lastID, query: cleanQuery });
            });
            stmt.finalize();
        });
    });
...
function getUserSearchHistory(user_id, limit = 10) {
    return new Promise((resolve, reject) => {
        db.all("SELECT id, query, created_at FROM search_history WHERE user_id = ? ORDER BY created_at DESC LIMIT ?", [user_id, limit], (err, rows) => {
            if (err) reject(err);
            else resolve(rows || []);
        });
    });
...
function deleteSearchHistoryItem(user_id, id) {
    return new Promise((resolve, reject) => {
        db.run("DELETE FROM search_history WHERE id = ? AND user_id = ?", [id, user_id], function (err) {
            if (err) reject(err);
...
function clearUserSearchHistory(user_id) {
    return new Promise((resolve, reject) => {
        db.run("DELETE FROM search_history WHERE user_id = ?", [user_id], function (err) {
            if (err) reject(err);
            else resolve({ success: true, changes: this.changes });
        });
    });
